...
- Contexte : on a un projet conforme aux bonnes pratiques de développement et portable (dockerisé)
- Mise en prod : faire vivre son application dans l'espace de ses utilisateurs
- Besoins : 
  - Déterminer le format de valorisation de son application (API, app interactive, site web..)
  - Déployer l'application dans un environnement de production
  - Industrialisation : automatisation des étapes, orchestration end-to-end du pipeline, monitoring..
- Solution : approche CI/CD
- Illustration des concepts : déploiement d'une API pour exposer un modèle simple de ML
- Qui déploie et comment ? Les différents rôles : Data Scientist, Data Engineer, Data Ops...
- Enjeu pour le data scientist : pas une maîtrise technique de tous les outils info, mais comprendre les concepts pour pouvoir jouer le rôle d'interface entre le métier et les équipes techniques (data engineer, ops..)
