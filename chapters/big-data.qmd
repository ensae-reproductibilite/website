...
```
1. Cette ligne de code utilise l'utilitaire Minio Client disponible sur le `SSPCloud`. Si vous n'êtes pas sur cette infrastructure, vous devrez remplacer cette ligne par une ligne téléchargeant cette donnée à partir de l'URL [https://projet-formation.minio.lab.sspcloud.fr/bonnes-pratiques/data/RPindividus.parquet](https://projet-formation.minio.lab.sspcloud.fr/bonnes-pratiques/data/RPindividus.parquet). Vous pouvez le faire en pur `Python` avec `requests` ou, si vous avez `curl` installé, par l'intermédiaire de `Python` avec la commande `os.system("curl -o data/RPindividus.parquet https://projet-formation/bonnes-pratiques/data/RPindividus.parquet")`.
...
:::{.application collapse="false" icon=false}
...
* Créons notre décorateur, en charge de _benchmarker_ le code `Python`:
  ...
  def measure_performance(func, return_output=False):
      ...
      def wrapper(return_output=False, *args, **kwargs):
          ...
          peak_mem_formatted = f"\033[92m{peak_mem:.2f} MB\033[0m"
...
On gagne déjà un temps conséquent en lecture mais on ne bénéficie pas vraiment de l'optimisation permise par `Parquet` car on transforme les données directement en `DataFrame` `Pandas`. On n'utilise donc pas l'une des fonctionnalités principales du format `Parquet` qui explique ses excellentes performances: le _predicatep pushdown_.
