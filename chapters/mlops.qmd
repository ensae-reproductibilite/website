...
![Source: [martinfowler.com](https://martinfowler.com)](/ML-model-lifecycle){fig-align="center"}
...
On voit bien qu'il peut y avoir plusieurs facette au terme surveillance. En l'occurence, on retient deux définitions de la surveillance : celle de l'informaticien et celle du data-scientist
...
Bonjour Jules !
Oui l'emménagement s'est bien déroulé et le lave linge est bien rentré à sa place en enlevant les charnières ! C'est top !
On est encore un peu au milieu des cartons et occupés avec l'autre appartement à rendre mais on a commencé à noter quelques points qu'on vous transmettra d'ici la fin de semaine quand on sera posé. 
Normalement on a bien répondu au questionnaire (depuis le lien reçu par mail, je ne sais pas si celui de slack est différent ?) mais on n'a pas encore pris le temps pour le commentaire google mais on fait ça dès qu'on est en vacances 
...
L'observabilité en temps réel des modèles de machine learning est cruciale pour garantir leur performance et leur fiabilité une fois déployés. Cette observabilité diffère significativement entre les besoins des informaticiens, qui se concentrent sur le monitoring des systèmes informatiques (comme la disponibilité des ressources, les performances de l'infrastructure, etc.), et ceux des data scientists ou des unités business, qui surveillent l'efficacité des modèles d'un point de vue métier. Les défis tels que le "Data Drift" (dérive des données) et le "Concept Drift" (dérive de concept) nécessitent une attention particulière, car ils peuvent indiquer que le modèle ne performe plus comme attendu en raison de changements dans les données ou dans le contexte d'application. Pour cela, il est essentiel de monitorer des métriques spécifiques, telles que la précision du modèle, le taux d'erreur, ou d'autres indicateurs clés de performance (KPI) alignés sur les objectifs business. Un système d'alerting bien configuré, couplé à un tableau de bord dynamique, permet aux équipes de réagir rapidement aux anomalies et d'assurer la maintenance proactive des modèles.

4️⃣ Réentraînement d'un modèle ML

Le réentraînement des modèles de machine learning est un aspect fondamental de leur cycle de vie, permettant une amélioration continue face à l'évolution des données et des exigences métier. Ce processus peut être abordé de manière périodique, où le modèle est réévalué et mis à jour à intervalles réguliers, ou de manière continue, intégrant un flux constant de nouvelles données pour un ajustement en temps réel. Les boucles de rétroaction (feedback loops) jouent un rôle essentiel dans ce contexte, facilitant une expérimentation agile, le déploiement rapide des améliorations et un monitoring efficace pour identifier les opportunités d'optimisation. Cette approche itérative assure que les modèles restent pertinents et performants, s'adaptant aux changements dans les comportements des utilisateurs et aux conditions du marché.
5️⃣ Défis organisationnels du MLOps

L'intégration du MLOps au sein des organisations présente des défis organisationnels notables, principalement en raison de la jeunesse relative des équipes data et de leur manque de ressources qualifiées pour gérer avec efficacité le déploiement et la maintenance de systèmes ML complexes. Bien que ces équipes comprennent souvent d'excellents data scientists, leur expertise se limite généralement au développement de modèles, sans s'étendre aux compétences nécessaires pour le déploiement et la gestion opérationnelle d'applications complètes. De surcroît, ces équipes tendent à opérer en silo, ce qui limite la collaboration et la communication efficace avec d'autres équipes techniques essentielles pour la mise en production réussie des modèles. Pour surmonter ces obstacles, il est crucial d'adopter une approche plus intégrée, encourageant le partage de connaissances et la collaboration interdisciplinaire, afin de bâtir des ponts entre le développement de modèles et leurs applications pratiques dans un cadre opérationnel.
