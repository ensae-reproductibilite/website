...
Grâce aux chapitres précédents, nous avons appris comment structurer et rendre notre projet de data science reproductible et portable en utilisant l'approche DevOps. Pour de nombreux projets de data science, cette approche peut être suffisante. Cependant, dès lors que nous intégrons un modèle de machine learning à notre projet, l'approche DevOps devient une condition nécessaire mais plus suffisante pour réussir à mettre notre modèle en production. À l'heure actuelle, la grande majorité des modèles de machine learning restent au stade de l'expérimentation, et très peu parviennent à passer en production. Pour faciliter cette transition vers la production, une nouvelle approche a émergé, prenant en compte les spécificités des modèles de machine learning : le MLOps. On peut considérer le MLOps comme la fusion de trois concepts clés, à savoir :

$$
MLOps = DataOps + DevOps + ModelOps
$$

Ce chapitre vise à définir cette approche et à présenter plusieurs outils pour l'adopter dans nos projets de data science.
...
L'approche DevOps, contraction des termes "Development" et "Operations", est une méthodologie de développement logiciel visant à améliorer la collaboration et la communication entre les équipes de développement (Dev) et d'administration système (Ops). Son objectif principal est d'accélérer et d'optimiser le cycle de vie du développement logiciel, de la conception à la production, en favorisant l'automatisation, l'intégration continue et le déploiement continu (voir le chapitre sur la mise en production).

Initiée en 2007, cette approche cherche à éliminer les barrières traditionnelles entre les équipes de développement et celle de l'informatique, favorisant ainsi une culture de responsabilité partagée. Ses principes fondamentaux incluent l'**automatisation** des processus, la **surveillance** continue des performances, la **gestion agile** des changements et la création d'une **boucle de rétroaction** rapide pour améliorer en permanence les processus de développement et d'exploitation.

En résumé, le DevOps vise à créer un écosystème de développement collaboratif, automatisé et orienté vers l'amélioration continue. Le cycle de vie DevOps est souvent décrit à travers ces 8 phases interconnectées : 
...
MLflow fournit un serveur de suivi qui comprend à la fois une interface graphique ergonomique ainsi qu'une API permettant de *logger* différents paramètres, métriques, fichiers, etc. durant l'entraînement de votre modèle de machine learning. Le *tracking server* est très utile pour comparer les différentes expérimentations que vous avez effectuées, pour les stocker et également pour être capable de les reproduire. En effet, chaque *run* sauvegarde la source des données utilisées, mais également le commit sur lequel le *run* est basé.
...
- l'automatisation de pipeline est beaucoup plus compliquée et peu lisible. Il existe cependant des packages qui permettent d'automatiser des *pipelines* de notebooks comme [Elyra](https://github.com/elyra-ai/elyra) par exemple, mais ce n'est clairement pas l'approche que nous vous recommandons.
- Les *workflows* sont souvent moins clairs et peu reproductibles, avec souvent une organisation peu optimale (toutes les fonctions définies dans le même fichier affectant la lisibilité du code par exemple).
