...
Pour les prochaines applications, on va supposer l'utilisation d'un système de stockage _cloud_ de type `S3` (pour en savoir plus sur l'intérêt de cette approche, se reporter au [chapitre dédié](/big-data.qmd)). 
...
La 2e voie est recommandé.  Le plus simple est de 1/ cliquer sur le bouton suivant  
...
De cette manière votre `VSCode` sera pré à l'emploi, à l'image de ce que vous aviez fait pour l'application 0.
...
Concrètement, nous allons adopter le pipeline suivant pour notre projet:

![](/chapters/applications/figures/pipeline_appli8.png)

Le scénario type de la première partie de l'application (appli8a). Dans celle-ci, nous avons une source brute, reçue sous forme de CSV, dont on ne peut changer le format. Il aurait été idéal d'avoir un format plus adapté au traitement de données pour ce fichier mais ce n'était pas de notre ressort. Notre chaine va aller chercher ce fichier, travailler dessus jusqu'à valoriser celui-ci sous la forme de notre matrice de confusion. Si on imagine que notre chaine prend un certain temps, il n'est pas inutile d'écrire des données intermédiaires. Pour faire cela, puisque nous avons la main, autant choisir un format adapté, à savoir le format `Parquet`.

Néanmoins, ici, nous avons une donnée qui ne change jamais. On n'a pas de mise à jour fréquente de données qui justifie de ne pas changer, une fois pour toute, notre format de données afin d'en adopter un plus pertinent. C'est l'objet de l'appli 8c. 
