...
Maintenant qu'on sait que ce script préparatoire fonctionne, on va le transformer en `Dockerfile` (la syntaxe `Docker` est légèrement différente de la syntaxe `Linux` classique) et le tester dans un environnement bac à sable ou sur des machines mises à disposition par les services d'intégration continue de `Github`.

### Préliminaire

- [Installer Docker](https://docs.docker.com/get-docker/) sur sa machine ou bien se rendre sur l'environnement bac à sable [Play with Docker](https://labs.play-with-docker.com)
...
- Ouvrir ce fichier via un éditeur de texte
...
L'étape de `build` a fonctionné: une image a été construite. Mais fait-elle effectivement ce que l'on attend d'elle ? Pour le savoir, il faut passer à l'étape suivante, l'étape de `run`.
