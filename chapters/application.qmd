...
- Forker le dépôt `Github` : https://github.com/linogaliana/ensae-reproductibilite-application-correction
...
## Etape 2: construire une application de A à Z via un script shell

Bien que la logique de la conteneurisation soit une brique de base
de toutes les plateformes _data science_ modernes, il est
compliqué de trouver une instance en ligne qui 
propose un environnement bac à sable permettant un usage
de `Docker`.

L'environnement [Play with Docker](https://labs.play-with-docker.com)
est intéressant mais est assez versatile. On va donc procéder différemment
en fonctionnant en deux étapes, quitte à perdre l'aspect interactif :

- Nous allons d'abord créer un script shell qui construit notre
application de _machine learning_ de A à Z ;
- Nous transformerons celui-ci en `Dockerfile` dans un deuxième temps. C'est l'objet
de l'étape suivante. 
...
2. Cloner le dépôt et se placer au niveau du checkpoint 11a avec `git checkout appli11a`
...
## Etape 3: conteneuriser avec `Docker` {#docker}


Maintenant qu'on sait que ce script préparatoire fonctionne,
on va le transformer en `Dockerfile` (la syntaxe `Docker`
est légèrement différente de la syntaxe `Linux` classique) et le tester dans
un environnement bac à sable ou sur des machines mises à disposition 
par les services d'intégration continue de `Github`. 

Plan:

1. Appli1: script en docker
2. Appli2 optionnel: tester dans l'environnement bac à sable
3. Appli3: GHA docker
