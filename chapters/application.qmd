...
est effectué à chaque lancement d'un nouveau conteneur. 
On relance donc autant de fois un entraînement qu'on déploie
de conteneur pour répondre à nos utilisateurs. Ce sera
...
à chaque changement de notre code. Autrement dit, on améliore
...
- Le __code__. C'est l'élément qui évolue chez nous. On va donc faire en sorte qu'à chaque mise à jour de notre code (un _push_ sur `Github`), les étapes ultérieures (production de l'image `Docker`, etc.) vont se mettre à compiler. 

![](https://inseefrlab.github.io/formation-mlops/slides/img/ci-cd.png)


Grâce à `ArgoCD` il est possible de déployer un modèle de manière continu, 
ainsi chaque modification d’un fichier présent dans le dossier `kubernetes/`. Cela
va entrainer le redéploiement automatique en synchronisant avec notre dépôt Github.


::: {.callout-tip}
## Projet d'application 19: Mettre à disposition l'API (déploiement automatisé)
