...
9. Créer le fichier `environment.yml` à partir de `conda env export`:
...
1. Créer un fichier `.github/workflows/test.yml` avec le contenu
de l'exemple de la documentation
2. N'utiliser que les versions 3.9 à 3.11 de `Python`
3. Utiliser le fichier `requirements.txt` pour installer les dépendances. 
Ajouter l'argument `cache: 'pip'` au moment de l'étape `actions/setup-python@v4`
([voir documentation](https://docs.github.com/en/actions/automating-builds-and-tests/building-and-testing-python#installing-dependencies))
pour mettre en _cache_ les dépendances afin de gagner du temps sur les installations
ultérieures
...
<details>
<summary>Fichier `.github/workflows/test.yml` obtenu</summary>
...
contenu du [_template_](https://github.com/marketplace/actions/build-and-push-docker-images) dedans ; 
...
- Si besoin, changer les branches sur lesquelles peut tourner
le pipeline ;
...
des valeurs de Shapley, les facteurs qui auraient pu nous mettre la puce
à l'oreille sur les destins de Jake et de Rose. 


::: {.callout-tip}

## Application 18: Déploiement automatisé d'un site web

Pour cela, on va utiliser `Quarto` et déployer sur `Github pages`.

- Installer `quarto`
- Utiliser `quarto create-project` et supprimer le fichier avec l'extension `.qmd` créé
- Récupérer le contenu du modèle de site web sur [cette page](https://raw.githubusercontent.com/ensae-reproductibilite/application-correction/tree/appli18/index.qmd) et mettre dans un fichier `index.qmd`
- Modifier fichier `train.py` à partir de [cette page](https://raw.githubusercontent.com/ensae-reproductibilite/application-correction/tree/appli18/train.py)
- En ligne de commande, faire `quarto preview` et observer le site web généré en local
- Créer une branche `gh-pages` à partir du contenu de [cette page](https://quarto.org/docs/publishing/github-pages.html)
- Créer un fichier `.github/workflows/website.yaml` avec le contenu de [ce fichier](https://raw.githubusercontent.com/linogaliana/ensae-reproductibilite-website/master/.github/workflows/publish.yaml)
