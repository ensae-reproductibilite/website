...
regarder la page Experiments. Elle est vide à ce stade, c'est normal
...
Retourner sur l'UI et observer la différence

4. Créer un fichier `src/models/log.py` dont le contenu
est le suivant

```{python}
#| eval: false
#| file: src/models/log.py
#| filename: src/models/log.py
import mlflow
import os

DATA_URL = os.environ["MLFLOW_TRACKING_URI"]
...
a. import src.models.log as tl
b. EXPERIMENT_NAME = "titanicml"
parser.add_argument(
    "--appli", type=str, default="appli21", help="Application number"
)
c. tl.log_gsvc_to_mlflow(pipe_cross_validation, EXPERIMENT_NAME, APPLI_ID)
...
```python
import mlflow
logged_model = 'runs:/5fb6b1f4ac404f6ebb723533a856a193/gscv_model'
...
5. Go back to the list of runs by clicking again on the "titanicml" experiment
6. onglet `Table`, sélectioonner plusieurs expérimentations  Add additional columns using the *Columns* drop-down menu in the *Table* panel
7. Sort the models in descending order according to the **mean test F1** 


Intérêt à ce stade: on a archivé (loggé) les performances pour les différents jeux
de paramètres de notre validation croisée
Pb: 
- on entraîne en local, de manière séquentielle
- on n'archive pas les jeux de données associées à ces modèles (train & test), on 
doit le faire manuellement, ce qui est pénible
- on récupère manuellement les modèles,