...
Copier un exemple de fichier `Parquet` mis à disposition dans votre bucket personnel,

```python
mc cp s3/projet-formation/bonnes-pratiques/data/REGION=11/part-0.parquet s3/$BUCKET_PERSONNEL/ensae-reproductibilite/data/example/rp.parquet
```

en remplaçant `$BUCKET_PERSONNEL` par la valeur voulue.

Tester ces exemples de code
...
df = pd.read_csv(f"s3://{MY_BUCKET}/{CHEMIN}", filesystem=fs)
...
s3 = fs(endpoint_override ="https://minio.lab.sspcloud.fr")

df = pq.read_table("{MY_BUCKET}/{CHEMIN}", filesystem=s3)
...
query_definition = f"SELECT * FROM read_parquet('s3://{MY_BUCKET}/{CHEMIN}')"
df = con.sql(query_definition)
