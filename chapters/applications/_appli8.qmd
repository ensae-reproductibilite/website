...
```{.python}
MY_BUCKET = "mon_nom_utilisateur_sspcloud" #<1>
CHEMIN = "ensae-reproductibilite/data/example/rp.parquet"
```
...
df = pd.read_parquet(f"s3://{MY_BUCKET}/{CHEMIN}", filesystem=fs)
...
df = pq.read_table(f"{MY_BUCKET}/{CHEMIN}", filesystem=s3)
