...
3. Ouvrir un notebook temporaire et observer le résultat.
    ...
    ```{.python file="Dans une cellule de notebook" code-fold="true" code-summary="Exemple de code à tester"}
    import mlflow
    import pandas as pd
...
4. On va adapter le code applicatif de notre API pour tenir compte de ce modèle de production.
    ...
    ```{.python file="app/api.py" code-fold="true" code-summary=""}
...
6. On va retirer l'entraînement de la séquence d'opération du `api/run.sh`. En supprimant la ligne relative à l'entraînement du modèle, vous devriez avoir
...
7. _Commit_ de ce changement dans `main`

8. Publier un tag `v0.0.3` pour le code applicatif

9. Mettre à jour notre manifeste dans le dépôt `GitOps`.
...
2. La variable d'environnement à adapter en fonction de l'adresse du dépôt de modèles utilisé. Remplacer par votre URL `MLFlow`.
