...
3. Créer la variable comme proposé ci-dessous et vérifier la sortie de `Python` en faisant tourner `titanic.py` en ligne de commande ce code
...
4. Cela devrait vous renvoyer une erreur, comprenez-vous pourquoi ? Maintenant introduire la valeur voulue pour le jeton d'API dans le fichier d'environnement lu par `dotenv`. 
5. S'il n'existe pas déjà, créer un fichier `.gitignore` (cf. [Chapitre `Git`](/chapters/git.qmd)). Ajouter dans ce fichier `.env` car il ne faut pas committer ce fichier. Au passage ajouter `__pycache__/` au `.gitignore`[^pycache], cela évitera d'avoir à le faire ultérieurement ;
