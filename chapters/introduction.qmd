...
- contrôler la qualité de son code en utilisant des outils dédiés (cf. chapitre [Qualité du Code](XXXXXXX)) ;
- adopter une structure de projet standardisée en utilisant des *templates* prêts à l'emploi (cf. chapitre [Architecture des Projets](XXXXX)) ;
- utiliser Git pour versionner le code de ses projets, qu'ils soient individuels ou collectifs (cf. chapitre [Versionner son code et travailler collaborativement avec Git](XXXXXXX)) ;
- contrôler les dépendances de son projet en développant dans des environnements virtuels (cf. chapitre [Portabilité](XXXXXXX)).
